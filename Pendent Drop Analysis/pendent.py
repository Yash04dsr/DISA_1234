...
from PyQt5.QtWidgets import (
    QApplication, QWidget, QVBoxLayout, QPushButton,
    QLabel, QHBoxLayout, QSizePolicy, QMessageBox,
    QFormLayout, QLineEdit, QScrollArea
)
from PyQt5.QtGui import QPixmap, QImage
from PyQt5.QtCore import Qt
...
class PendantDropAnalyzer(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Pendant Drop Surface Tension Analyzer (Auto Load: ~/pendant.png)")
        self.setGeometry(100, 100, 1000, 900)
        ...
        layout = QVBoxLayout(self)
        self.image_label = QLabel("Loading pendant.png from home directory...")
        self.image_label.setAlignment(Qt.AlignCenter)
        self.image_label.setStyleSheet("border: 2px solid #888;")
        self.image_label.setMinimumSize(720, 460)
        self.image_label.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
    ...
    def load_default_image(self):
        path = os.path.expanduser("~/pendant.png")
        if not os.path.exists(path):
            self.image_label.setText("Error: ~/pendant.png not found.")
            return
        img = cv2.imread(path)
        if img is None:
            self.image_label.setText("Error: Could not load pendant.png.")
            return
        h, w = img.shape[:2]
        if h > 700:
            img = cv2.resize(img, (int(w * 700 / h), 700), interpolation=cv2.INTER_AREA)
        self.img = img
        self.display_image(img)
        self.result_label.setText("Click 'Analyze' to start surface tension analysis.")
    ...
    def run_analysis(self):
        if self.img is None:
            QMessageBox.warning(self, "Error", "No image loaded (~/pendant.png missing?).")
            return
...
class ScrollablePendantAnalyzer(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Pendant Drop Analyzer (Auto-load ~/pendant.png)")
        self.setGeometry(100, 100, 800, 600)
