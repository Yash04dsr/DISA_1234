...
def measure_drop_diameters(cnt):
    ...
    y_eq = ys[np.argmax(xs) if len(xs) > 0 else 0]
...
def calculate_surface_tension(D_E, D_S, density, scale_mm_per_px):
    ...
    ratio = D_S_mm / D_E_mm
    shape_factor = 0.5 + 1.5 * ratio  # empirical
    gamma = density * g * (D_E_mm ** 2) / (shape_factor * 1000)  # mN/m
    return gamma, shape_factor
...
class PendantDropAnalyzer(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Pendant Drop Analyzer – Visual Method (D_E & D_S)")
        ...
        self.params_label = QLabel("")  # additional parameters info
    ...
    def run_analysis(self):
        ...
        try:
            ...
            scale_mm_per_px = needle_diameter_mm / 50.0
            density = 998  # kg/m³ (for water)

            gamma, shape_factor = calculate_surface_tension(D_E, D_S, density, scale_mm_per_px)
            ...
            if gamma is None:
                ...
                params = (
                    f"<b>Parameters:</b> ρ = {density} kg/m³, g = 9.81 m/s², "
                    f"scale = {scale_mm_per_px:.4f} mm/px<br>"
                    f"Shape factor (H) ≈ 0.5 + 1.5×(D_S/D_E) = {shape_factor:.3f}"
                )
            ...
            ys = cnt[:, 0, 1]
            xs = cnt[:, 0, 0]
            y_eq_line = np.median(ys)
            x_min, x_max = np.min(xs), np.max(xs)
            cv2.line(vis, (x_min, int(y_eq_line)), (x_max, int(y_eq_line)), (255, 0, 0), 2)
            cv2.putText(vis, "D_E (max width)", (x_max + 10, int(y_eq_line)),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 0), 2)

            # Draw D_S (red line)
            if y_ds is not None:
                xs_band = cnt[(ys >= y_ds - 2) & (ys <= y_ds + 2), 0, 0]
                if len(xs_band) > 1:
                    x_min2, x_max2 = np.min(xs_band), np.max(xs_band)
                    cv2.line(vis, (x_min2, y_ds), (x_max2, y_ds), (0, 0, 255), 2)
                    cv2.putText(vis, "D_S (¼ height)", (x_max2 + 10, y_ds),
